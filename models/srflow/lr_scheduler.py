...
#     def __init__(self, optimizer, T_period, restarts=None, weights=None, eta_min=0, __last_epoch=-1):
...
#         assert len(self.restarts) == len(
#             self.restart_weights), 'restarts and their weights do not match.'
#         super(CosineAnnealingLR_Restart, self).__init__(optimizer, __last_epoch)
#
#     def get_lr(self):
#         if self.__last_epoch == 0:
#             return self.base_lrs
#         elif self.__last_epoch in self.restarts:
#             self.last_restart = self.__last_epoch
#             self.T_max = self.T_period[self.restarts.index(self.__last_epoch) + 1]
#             weight = self.restart_weights[self.restarts.index(self.__last_epoch)]
#             return [group['initial_lr'] * weight for group in self.optimizer.param_groups]
#         elif (self.__last_epoch - self.last_restart - 1 - self.T_max) % (2 * self.T_max) == 0:
#             return [
#                 group['lr'] + (base_lr - self.eta_min) * (1 - math.cos(math.pi / self.T_max)) / 2
#                 for base_lr, group in zip(self.base_lrs, self.optimizer.param_groups)
#             ]
#         return [(1 + math.cos(math.pi * (self.__last_epoch - self.last_restart) / self.T_max)) /
#                 (1 + math.cos(math.pi * ((self.__last_epoch - self.last_restart) - 1) / self.T_max)) *
