...
from models.srflow.modules import thops
from models.srflow.modules.FlowUpsamplerNet import FlowUpsamplerNet
from models.srflow.modules.RRDBNet_arch import RRDBNet
from models.srflow.modules.flow import GaussianDiag
from models.srflow.utils.util import opt_get


class SRFlowNet(nn.Module):
    def __init__(self, lr_size, hr_size, in_nc, out_nc, nf, nb, gc=32, scale=4, K=None, opt=None, step=None):
        super(SRFlowNet, self).__init__()

        self.opt = opt
        self.quant = opt_get(opt, ['datasets', 'train', 'quant'])
        ...
        self.flowUpsamplerNet = \
            FlowUpsamplerNet((hr_size, hr_size, out_nc), hidden_channels, K,
    ...
    def normal_flow(self, gt, lr, y_onehot=None, epses=None, lr_enc=None, add_gt_noise=True, step=None):
        ...
        if add_gt_noise:
            # Setup
            noiseQuant = opt_get(self.opt, ['network_G', 'flow', 'augmentation', 'noiseQuant'])
            if noiseQuant:
