...
from models.srflow.modules.module_util import initialize_weights, make_layer
...
class RRDBNet(nn.Module):
    ...
    def forward(self, x, get_steps=False):
        ...
        if fea_up0_en:
            results['fea_up0'] = F.interpolate(last_lr_fea, scale_factor=1/2, mode='bilinear', align_corners=False,
                                               recompute_scale_factor=True)
        fea_upn1_en = opt_get(self.opt, ['network_G', 'flow', 'fea_up-1']) or False
        if fea_upn1_en:
            results['fea_up-1'] = F.interpolate(last_lr_fea, scale_factor=1/4, mode='bilinear', align_corners=False,
                                                recompute_scale_factor=True)
