...
from models.srflow.modules.FlowStep import FlowStep
from models.srflow.modules.Split import Split2d
from models.srflow.modules.flow import SqueezeLayer
from models.srflow.modules.glow_arch import f_conv2d_bias
from models.srflow.utils.util import opt_get
...
class FlowUpsamplerNet(nn.Module):
    def __init__(self, image_shape, hidden_channels, K, L=None,
        ...
        self.H = H
        self.W = W
        self.scaleH = self.hr_shape / H
        self.scaleW = self.hr_shape  / W
    ...
    def arch_FlowStep(self, H, K, LU_decomposed, W, actnorm_scale, affineInCh, flow_coupling, flow_permutation,
        ...
        for k in range(K):
            position_name = get_position_name(self.hr_shape, H, self.opt['scale'])
    ...
    def arch_split(self, H, W, L, levels, opt, opt_get):
        correct_splits = opt_get(opt, ['network_G', 'flow', 'split', 'correct_splits'])
        ...
        if opt_get(opt, ['network_G', 'flow', 'split', 'enable']) and L < levels - correction:
            ...
            t = opt_get(opt, ['network_G', 'flow', 'split', 'type'])
    ...
    def encode(self, gt, rrdbResults, logdet=0.0, epses=None, y_onehot=None):
        ...
        for layer, shape in zip(self.layers, self.output_shapes):
            size = shape[2]
            level = int(np.log(self.hr_shape / size) / np.log(2))
...
def get_position_name(hr_shape, H, scale):
    downscale_factor = hr_shape // H
