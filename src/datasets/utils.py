...
def check_img_files(img_files: pd.DataFrame, shape: Tuple[int, int],
    ...
    for base_name, files in img_files.iterrows():
        for exp, pathlist in files.items():
...
def apply_transform(img: Union[torch.Tensor, List[torch.Tensor]], transforms: List[Callable]):
    if type(img) == list:
...
def load_det_mask(res_mult: int):
    with fits.open(Path("res") / "detector_mask" / f"pn_mask_500_2000_detxy_{res_mult}x.ds") as hdu:
...
def filter_img_dict(
        img_dict: Dict[int, List[Path]],
        base_names: set,
        split_key: str
) -> Dict[int, Dict[str, List[str]]]:
    filtered_img_dict = {exp: {
        base_name: [] for base_name in base_names
    } for exp in img_dict.keys()}
...
def match_file_list(
    ...
    Args:
        lr_dict: List of files1 or list of lists
        hr_dict: List of files2
