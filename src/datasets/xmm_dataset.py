...
from datasets.utils import get_fits_files, match_file_list, reshape_img_to_res, load_fits, group_same_sources


class XmmDataset(Dataset):
    """XMM-Newton dataset"""

    def __init__(self, dataset_name, datasets_dir, split, dataset_lr_res, lr_exp, hr_exp, det_mask, exp_channel, include_hr,
                 check_files=False, transform=None, normalize=None):
        """
        Args:
        ...
        if self.include_hr:
            ...
            self.lr_img_files, self.hr_img_files, self.base_img_files = group_same_sources(self.lr_img_files,
                                                                                           self.hr_img_files,
                                                                                           split_key="_image_split_")
        else:
            self.lr_img_files, _, self.base_img_files = group_same_sources(self.lr_img_files,
                                                                           self.lr_img_files[-1],
                                                                           split_key="_image_split_")
        ...
        if self.det_mask or self.exp_channel:
            ...
            lr_det_mask_base_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'detector_mask',
            ...
            if self.include_hr:
                self.hr_det_mask = self.lr_det_mask  # This is on the same resolution, thus the same detmask
    ...
    def load_xmm_sample(self, idx):
        ...
        lr_img['img'] = reshape_img_to_res(dataset_lr_res=self.dataset_lr_res, img=lr_img['img'], res_mult=1)

        if self.include_hr:
            hr_img['img'] = reshape_img_to_res(dataset_lr_res=self.dataset_lr_res, img=hr_img['img'], res_mult=1)

        if exp_channel is not None:
            # Add the exposure channel
            lr_img['exp_channel'] = reshape_img_to_res(dataset_lr_res=self.dataset_lr_res, img=exp_channel,
                                                       res_mult=1)
    ...
    def __getitem__(self, idx):
        ...
        sample = {'lr': lr_img, 'lr_exp': lr_img_sample['exp'],
                  'lr_img_file_name': lr_img_sample['file_name'], 'lr_header': lr_img_sample['header']}
