...
class LitESRGAN(pl.LightningModule):
    def __init__(
        ...
        self.discriminator = Discriminator(
            input_shape=(out_channels, *hr_shape),
            filter_start_size=discriminator_filter_start,
        )
    ...
    def _generator_loss(self, batch):
        ...
        valid = Variable(
            torch.ones(
                (imgs_lr.size(0), *self.discriminator.output_shape), device=self.device
            ),
            requires_grad=False,
        )
        ...
        loss_G = (
            self.hparams["lambda_adv"] * loss_GAN
            + self.hparams["lambda_pixel"] * loss_pixel
            + loss_content
        )
