...
model_config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'configs', 'model',
...
config['wandb_log_online'] = config[
    'log_online']  # not config['fast_dev_run'] and not config['debug'] # Only log the full runs
...
def main(config):
    ...
    sim_display_config['datasets_dir'] = os.path.join(sim_display_config['datasets_dir'], 'display_datasets')
    ...
    real_display_config['datasets_dir'] = os.path.join(real_display_config['datasets_dir'], 'display_datasets')
    # Override the lr_exp to always use the same exposure, otherwise the display images would be inconsistent
    ...
    train_datamodule = XmmDataModule(config['dataset_config'], split='train' + split_name_add)
    train_dataloader = train_datamodule.get_dataloader(shuffle=True)

    print("Setting up val dataloader")
    val_datamodule = XmmDataModule(config['dataset_config'], split='val' + split_name_add)
    val_dataloader = val_datamodule.get_dataloader(shuffle=False)

    if config['run_test_dataset']:
        print("Setting up test dataloader")
        test_datamodule = XmmDataModule(config['dataset_config'], split='test' + split_name_add)
    ...
    for i in range(len(sim_display_dataloader)):
        sim_display_samples.append(next(sim_display_dataloader))  # This will return a dict

    for i in range(len(real_display_dataloader)):
        real_display_samples.append(next(real_display_dataloader))  # This will return a dict
    ...
    criterion = lfh.get_loss_f(l1_p=config['model_config']['loss_l1'], poisson_p=config['model_config']['loss_poisson'],
                               psnr_p=config['model_config']['loss_psnr'], ssim_p=config['model_config']['loss_ssim'],
    ...
    if  config['model_config']['base_model'] == 'esr_gen':
        ...
        model = LitRRDBDenoise(lr_shape=config['model_config']['lr_shape'],
                               hr_shape=config['model_config']['hr_shape'],
                               in_channels=config['model_config']['in_channels'],
                               out_channels=config['model_config']['out_channels'],
                               filters=config['model_config']['filters'],
    ...
    for s_mode in ['linear', 'sqrt', 'asinh', 'log']:
        scaling_normalizers.append(Normalize(lr_max=config['dataset_config']['lr_max'], hr_max=config['dataset_config']['hr_max'], stretch_mode=s_mode ))

    callbacks += [
        ImageLogger(real_samples=real_display_samples, sim_samples=sim_display_samples,
                    log_every_n_epochs=config['log_images_every_n_epoch'], normalize= val_datamodule.normalize, scaling_normalizers=scaling_normalizers,
                    data_range=config['dataset_config']['hr_max']),
        OnnxExporter(in_dims=sim_display_samples[0]['lr'].shape, wandb_logger=wandb_logger),
        MetricLogger(normalize=val_datamodule.normalize, scaling_normalizers=scaling_normalizers, data_range=config['dataset_config']['hr_max']),
        checkpoint_callback]  # see Callbacks section

    if config['resume_from_checkpoint']:
        # Resume from checkpoint
        trainer = Trainer(resume_from_checkpoint=config['resume_model_checkpoint_path'], logger=wandb_logger,
                          # W&B integration
    ...
    # # evaluate the model on the test set
    trainer.test(model, dataloaders=test_dataloader,
                 ckpt_path='best')  # uses last-saved model
...
if __name__ == '__main__':  # This is needed in order to run it on multiple gpu's
    ...
    if not config['resume_run']:
        wandb.init(project=config['project'], dir=config['runs_dir'], entity=config['wandb_entity'], mode=wandb_mode, config=config)
    else:
        wandb.init(project=config['project'], dir=config['runs_dir'], entity=config['wandb_entity'], mode=wandb_mode, config=config, resume=config['resume_id'])
