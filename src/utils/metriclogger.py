...
class MetricLogger(pl.Callback):
    ...
    def log_metrics(self, input, gen, label, pl_module, stage, name_ext='', tng_set=None, log_inputs=False, log_extended=False):
        # Get the metrics, format is a dict
        metrics_scaled = self.get_metrics(input, gen, label, tng_set, log_inputs, log_extended)
    ...
    def _get_metric_score(self, input, gen, label, data_range, tng_set=None, log_input=False, log_extended=False):
        ...
        if log_input:
            psnr_in = piq.psnr(input, label,
                               data_range=data_range).detach().item()  # Relative psnr in comparison to the input
        ...
        ssim = get_ssim(gen, label, win_size=winsize, win_sigma=sigma,
                        data_range=data_range, K=K)[0].detach().item()
        if log_input:
            ssim_in = get_ssim(input, label, win_size=winsize, win_sigma=sigma,
                               data_range=data_range, K=K)[0].detach().item()

        if gen.shape[-1] > 192:
            # We can only calculate ms_ssim if the size is bigger than 192 due to the different scales in ms_ssim
            ms_ssim = get_ms_ssim(gen, label, win_size=winsize, win_sigma=sigma,
                                  data_range=data_range, weights=ms_weights, K=K).detach().item()

            if log_input:
                ms_ssim_in = get_ms_ssim(input, label, win_size=winsize, win_sigma=sigma,
                                         data_range=data_range, weights=ms_weights, K=K).detach().item()
        ...
        if log_input:
            poisson_in = nn.PoissonNLLLoss(log_input=False)(input, label).detach().item()
        ...
        if log_extended:
            ...
            if log_input:
                vif_p_in = piq.vif_p(x=input, y=label, data_range=data_range).detach().item()

            fsim = piq.fsim(x=gen, y=label, data_range=data_range, chromatic=False).detach().item()
            if log_input:
                fsim_in = piq.fsim(x=input, y=label, data_range=data_range, chromatic=False).detach().item()
            ...
            ms_gmsd = piq.multi_scale_gmsd(gen, label, data_range=data_range, chromatic=False).detach().item()
            if log_input:
                ms_gmsd_in = piq.multi_scale_gmsd(input, label, data_range=data_range, chromatic=False).detach().item()


            haarpsi = piq.haarpsi(x=gen, y=label, data_range=data_range).detach().item()
            if log_input:
                haarpsi_in = piq.haarpsi(x=input, y=label, data_range=data_range).detach().item()

            mdsi = piq.mdsi(x=gen, y=label, data_range=data_range).detach().item()
            if log_input:
                mdsi_in = piq.mdsi(x=input, y=label, data_range=data_range).detach().item()
    ...
    def get_metrics(self, input, gen, label, tng_set=None, log_inputs=False, log_extended=False):
        ...
        if input.shape[-1] != gen.shape[-1]:
            if not self.upsample:
                ...
                self.upsample = ImageUpsample(scale_factor=int(gen.shape[-1] / input.shape[-1]))
        ...
        for i in range(input.shape[0]):
            ...
            for scale_normalizer in self.scaling_normalizers:
                ...
                scaled_metrics[stretch_name] = self._get_metric_score(torch.unsqueeze(in_s, 0),  # Note that the input has been scaled up here
                                           torch.unsqueeze(gen_s, 0),
                                           torch.unsqueeze(label_s, 0),
                                           data_range=1.0, tng_set=tng_select, log_input=log_inputs,
    ...
    def log_metrics_batch_end(self, trainer, pl_module, outputs, batch, stage, log_inputs=False, log_extended=False):
        # input = batch['lr_gt']
        input = batch['lr']
        gen = outputs
        ...
        if len(input.shape) >= 4 and input.shape[1] >= 2:
            ...
            input = input[:, 0, :, :].reshape(input.shape[0], 1, input.shape[2], input.shape[3])
        ...
        if type(gen) == dict:
            ...
            for key in gen:
                name_ext = f"_h_{key}"
                self.log_metrics(input, gen[key], label, pl_module, stage=stage, name_ext=name_ext, tng_set=tng_set, log_inputs=log_inputs, log_extended=log_extended)
        else:
            self.log_metrics(input, gen, label, pl_module, stage=stage, tng_set=tng_set, log_inputs=log_inputs, log_extended=log_extended)
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        ...
        if trainer.current_epoch == 0:
            ...
            log_inputs = False
        self.log_metrics_batch_end(trainer=trainer, pl_module=pl_module, outputs=outputs, batch=batch, stage="val",
                                   log_inputs=log_inputs, log_extended=False)


    def on_test_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        self.log_metrics_batch_end(trainer=trainer, pl_module=pl_module, outputs=outputs, batch=batch, stage="test",
                                   log_inputs=True, log_extended=True)