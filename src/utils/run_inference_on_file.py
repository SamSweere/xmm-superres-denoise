...
from datasets.utils import load_det_mask, apply_transform
from datasets.utils import reshape_img_to_res, load_fits
from models import Model
from transforms import Crop
from transforms import Normalize
from transforms.totensor import ToTensor
from utils.filehandling import write_xmm_file_to_fits_wcs, read_yaml
...
def _infer_from_onnx(img: torch.Tensor, checkpoint_path: Path) -> torch.Tensor:
    ...
    ort_outs = ort_session.run(["name"], {"input": img[None, None, :].detach().cpu().numpy()})
...
def run_on_file(
    ...
    if plot:
        with fits.open(out / f"{in_name}.fits.gz") as f1, fits.open(out / f"{pred_name}.fits.gz") as f2:
            img_in = f1[0].data
            img_out = f2[0].data
        norm = ImageNormalize(img_in, interval=PercentileInterval(99.5))
        plt.imshow(img_in, norm=norm, cmap=colormaps["plasma"], origin='lower', interpolation='nearest')
        plt.savefig(out / "plot_in.png")
        norm = ImageNormalize(img_out, interval=PercentileInterval(99.5))
        plt.imshow(img_out, norm=norm, cmap=colormaps["plasma"], origin='lower', interpolation='nearest')
...
def _infer_file(
    ...
    if ontime >= 25.0 or ontime <= 15.0:
        warn(f'The networks were trained on 20 ks exposure images, '
    ...
    transform = [Crop(crop_p=1.0,  # TODO
                      mode=dataset_config["crop_mode"]),
                 ToTensor()]
    normalize = Normalize(lr_max=dataset_config["lr"]["max"], hr_max=dataset_config["hr"]["max"],
    ...
    if checkpoint_path.suffix == ".onnx":
        ...
        output = _infer_from_ckpt(checkpoint_path=checkpoint_path, model_config=model_config, img=img,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--fits', type=Path, help='The FITS filename in detxy coordinates with WCS', required=True)
    parser.add_argument("--checkpoint", type=Path, help="Path to the checkpoint file", required=True)
    parser.add_argument("--out", type=Path, help="Path where the created files should be stored", required=True)
