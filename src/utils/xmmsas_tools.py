...
# %%
def filter_events_gti(event_list, gti_file, max_expo=-1.0, pps_dir=os.getcwd(), output_name=None,
                      filter_expression=None, verbose=False):
    '''
    PURPOSE:
    ...
    #
    sas_args = [
        f'evselect table={event_list} withfilteredset=Y filteredset={output_name} destruct=Y keepfilteroutput=Y expression="{expr}"']
    #
    status = run_sas_command(sas_args)
    if (status.returncode != 0):
...
# %%
def make_detxy_image(event_list, pps_dir=os.getcwd(), output_name=None, low_energy=500, high_energy=2000, bin_size=80,
                     radec_image=True, verbose=False):
    '''
    PURPOSE:
