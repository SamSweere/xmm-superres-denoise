...
def run_sas_command(command, verbose=False):
    ...
    try:
        ...
        if retcode < 0:
            raise RuntimeError(f"Execution of {command} was terminated by signal {retcode}\n {result.stdout.decode()}")
        else:
            if verbose:
...
def get_pps_nxsa(
    ...
    if pps_dir.exists() and skip:
        print(f"Will skip PPS download for {obs_id} as {pps_dir} already exists and skip flag is set.")
    else:
        if pps_dir.exists():
            warn(f'Found an already existing folder {pps_dir} and skip flag is not set => files will be overwritten!')
        pps_tar_file = w_dir / "PPS_nxsa.tar"
        xmm.download_data(obs_id, level="PPS", extension="FTZ", filename=pps_tar_file.absolute().as_posix())
        if not tarfile.is_tarfile(pps_tar_file):
            raise RuntimeError(f"Downloaded file from NXSA {pps_tar_file} does not look like tar file.")
...
def make_gti_pps(
    ...
    for j in pps_files['fbk_files']:
        if (instrument.upper() in j.stem.upper()) or (instrument.upper() == 'ALL'):
            ...
            sas_args = [f'tabgtigen table={j} expression="RATE<={rate_lim}" gtiset={gti_name.absolute().as_posix()} '
                        f'mingtisize=50.0']
            ...
            if plot_it:
                ...
                if multi_plot:
                    ...
                    ax[k].axhline(rate_lim, color='red', linewidth=3, linestyle='dashed',
                                  label=f'GTI threshold {rate_lim:.2f} cts/s', zorder=2)
                    ...
                    ax.axhline(rate_lim, color='red', linewidth=3, linestyle='dashed',
                               label=f'GTI threshold {rate_lim:.2f} cts/s', zorder=2)
                    #
                    # and now the GTI bands
                    for jj in np.arange(ngti):
...
def filter_events_gti(
    ...
    sas_args = [f'evselect table={event_list} withfilteredset=Y filteredset={output_name.absolute().as_posix()} '
...
def make_detxy_image(
    ...
    sas_args = [f'ecoordconv imageset={output_name.absolute().as_posix()} x=0 y=0 coordtype=det']
    status = run_sas_command(sas_args)
