...
def run_inference_on_file(fits_file, dataset_config, onnx_filepath):
    ...
    transform = [
        Crop(
            crop_p=dataset_config["lr_res"] / dataset_config["dataset_lr_res"],
    ...
    for x in dataloader:
        ...
        #
        img_in_denorm = (
            normalize.denormalize_lr_image(torch.tensor(img_in[0][0])).detach().numpy()
        )
        img_in_denorm = img_in_denorm * exposure_in
        img_out_denorm = (
            normalize.denormalize_hr_image(torch.tensor(img_out)).detach().numpy()
        )
