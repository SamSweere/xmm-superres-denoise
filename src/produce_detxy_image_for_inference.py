...
from utils.xmmsas_tools import make_gti_pps, filter_events_gti, make_detxy_image, get_pps_nxsa
...
def get_detxy_for_obs_id(
    ...
    gtis = make_gti_pps(pps_files=pps_files, instrument=instrument, out_dir=proc_dir, max_expo=max_expo, plot_it=True,
                        save_plot=png_name)
    ...
    evl_filtered = filter_events_gti(evl, pn_gti, pps_files=pps_files, verbose=True,
                                     output_name=f'{instrument}_cleaned_evl_{suffix}', w_dir=obs_dir)

    if evl_filtered is None:
        raise RuntimeError("No filtered event lists found!")

    det_xy = make_detxy_image(event_list=evl_filtered, pps_dir=pps_dir, pps_files=pps_files, low_energy=500,
                              high_energy=2000, bin_size=80, radec_image=False, verbose=True,
                              output_name=f'{instrument}_500_2000_detxy_image_{suffix}', w_dir=obs_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--wdir', type=Path, default=Path.cwd(),
