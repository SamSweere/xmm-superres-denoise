...
_img_val_dict = {
    ...
    "generated": {"cm": "plasma", "norm": False, "key": "{0}/generated/{1}"},
    "label": {"cm": "plasma", "norm": False, "key": "{0}/label/{1}"},
    "difference": {"cm": "seismic", "norm": True, "key": "{0}/difference/{1}"},
    "ssim": {"cm": "seismic", "norm": True, "key": "{0}/ssim map/{1}"},
}
...
def _convert_images_to_cm(colormap, images, scale=None, normalize=False):
    images_cm = [
        _apply_cm(colormap, image.squeeze().cpu(), scale=scale, normalize=normalize)
        for image in images
    ]
    return images_cm
...
class ImageLogger(pl.Callback):
    ...
    @rank_zero_only
    def _get_samples(
        self, pl_module: "pl.LightningModule", dataloaders: EVAL_DATALOADERS
    ...
    def _log_images(
        ...
        outputs: Dict[str, Union[torch.Tensor, List[str]]] = self._get_samples(
            pl_module, dataloaders
        )
    ...
    def on_validation_end(
        ...
        if not trainer.sanity_checking:
            ...
            if ((trainer.current_epoch + 1) % self.log_every_n_epochs) == 0:
                self._log_images(
                    trainer.logger, pl_module, self.datamodule.val_dataloader(), "val"
                )
    ...
    def on_predict_batch_end(
        ...
        imgs = [
            _apply_cm(
                _img_val_dict["input"]["cm"],
                lr.cpu(),
                normalize=_img_val_dict["input"]["norm"],
            ),
            _apply_cm(
                _img_val_dict["generated"]["cm"],
                preds.cpu(),
                normalize=_img_val_dict["generated"]["norm"],
            ),
        ]
        captions = ["lr", "gen"]
        if "hr" in batch:
            imgs.append(
                _apply_cm(
                    _img_val_dict["label"]["cm"],
                    hr.cpu(),
                    normalize=_img_val_dict["label"]["norm"],
                )
            )
            ...
            imgs.append(
                _apply_cm(
                    _img_val_dict["difference"]["cm"],
                    difference_image.cpu(),
        ...
        trainer.logger.log_image(
            key=f"pred/{filenames[0][:60]}", images=imgs, caption=captions
        )
