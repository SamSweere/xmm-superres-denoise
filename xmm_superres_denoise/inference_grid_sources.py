...
def main(clamping_tr_nn, base_path, run_config, checkpoint, downscales, br_diffs_scales, shift_amounts, res = 1, exp = 20):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--clamping_tr_nn', type=str, default='deblending_original_clamp', help='Name of the training run')
    parser.add_argument('--base_path', type=str, default='shifted_grid_sources/results/', help='Base directory for results.')
    parser.add_argument('--run_config', type=Path, default=Path('res/baseline_config.yaml'), help='Path to run configuration YAML file.')
    parser.add_argument('--checkpoint', type=Path, default=Path('models/deblend_original_clamp.ckpt'), help='Path to model checkpoint file.')
